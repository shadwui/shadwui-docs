---
title: Switch
description: A control that allows the user to toggle between checked and not checked.
---

import {
  Tab,
  DocTabs,
  Tabs,
  TabsList,
  TabsContent,
  TabsTrigger,
  Step,
  Steps,
  CodeBlock,
  Separator,
  BannerDemo,
  CodePreview,
  ComponentPage,
  CodeBlockWrapper,
} from "../../components/index";

## Preview
<CodePreview
  sourcePath="src/default"
  componentName={"switch"}
  minimumCodeHeight="300px"
/>

## Installation 
<Tabs defaultValue="cli">
  <TabsList className="pb-0 h-10 rounded-md bg-hidden">
    <TabsTrigger
      value="cli"
      className="rounded-sm px-2 py-1.5 data-[state=active]:text-chai data-[state=active]:underline data-[state=active]:underline-offset-10 data-[state=active]:decoration-3 hover:cursor-pointer data-[state=active]:bg-hidden data-[state=active]:shadow-none"
    >
      CLI
    </TabsTrigger>
    <TabsTrigger
      value="manual"
      className="rounded-sm px-2 py-1.5 data-[state=active]:text-chai data-[state=active]:underline data-[state=active]:underline-offset-10 data-[state=active]:decoration-3 hover:cursor-pointer data-[state=active]:bg-hidden data-[state=active]:shadow-none"
    >
      Manual
    </TabsTrigger>
  </TabsList>
  <Separator />
  <TabsContent value="cli" className="p-0 mt-0">
    <DocTabs items={["pnpm", "npm", "yarn", "bun"]}>
      <Tab value="pnpm">
        ```bash
        pnpm dlx shadcn@latest add http://shadwui.com/r/switch.json
        ```
      </Tab>
      <Tab value="npm">
        ```bash
        npx shadcn@latest add http://shadwui.com/r/switch.json
        ```
      </Tab>
      <Tab value="yarn">
        ```bash
        npx shadcn@latest add http://shadwui.com/r/switch.json
        ```
      </Tab>
      <Tab value="bun">
        ```bash
        bunx --bun shadcn@latest add http://shadwui.com/r/switch.json
        ```
      </Tab>
    </DocTabs>
  </TabsContent>

  <TabsContent value="manual" className="p-0 mt-0">
   <Steps>
    <Step>
      **Install the following dependencies:**

      <DocTabs items={["pnpm", "npm", "yarn", "bun"]}>
        <Tab value="npm">
          ```bash
          pnpm add @radix-ui/react-switch
          ```
        </Tab>
        <Tab value="npm">
          ```bash
          npx install @radix-ui/react-switch
          ```
        </Tab>
        <Tab value="yarn">
          ```bash
          yarn add @radix-ui/react-switch
          ```
        </Tab>
        <Tab value="bun">
          ```bash
          bun add @radix-ui/react-switch
          ```
        </Tab>
      </DocTabs>

    </Step>
    <Step>
      **Copy and paste the following code into your project.**
      <CodeBlockWrapper>

      ```typescript
      "use client"

      import * as React from "react"
      import * as SwitchPrimitives from "@radix-ui/react-switch"

      import { cn } from "@/lib/utils"

      const Switch = React.forwardRef<
        React.ElementRef<typeof SwitchPrimitives.Root>,
        React.ComponentPropsWithoutRef<typeof SwitchPrimitives.Root>
      >(({ className, ...props }, ref) => (
        <SwitchPrimitives.Root
          className={cn(
            "peer inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:bg-primary data-[state=unchecked]:bg-input",
            className
          )}
          {...props}
          ref={ref}
        >
          <SwitchPrimitives.Thumb
            className={cn(
              "pointer-events-none block h-5 w-5 rounded-full bg-background shadow-lg ring-0 transition-transform data-[state=checked]:translate-x-5 data-[state=unchecked]:translate-x-0"
            )}
          />
        </SwitchPrimitives.Root>
      ))
      Switch.displayName = SwitchPrimitives.Root.displayName

      export { Switch }

      ```
      </CodeBlockWrapper>
    </Step>

     <Step>
        **Update the import paths to match your project setup.**
    </Step>
   </Steps>
  </TabsContent>
</Tabs>

## Usage
```jsx
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/badge"
```

## Examples
### 01
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-01"}
  minimumCodeHeight="250px"
  codeBlockMaximumHeight="250px"
/>

### 02
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-02"}
  codeBlockMaximumHeight="250px"
  minimumCodeHeight="250px"
/>

### 03
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-03"}
  codeBlockMaximumHeight="250px"
  minimumCodeHeight="250px"
/>

### 04
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-04"}
  codeBlockMaximumHeight="250px"
  minimumCodeHeight="250px"
/>

### 05
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-05"}
  codeBlockMaximumHeight="250px"
  minimumCodeHeight="250px"
/>

### 06
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-06"}
  codeBlockMaximumHeight="250px"
  minimumCodeHeight="250px"
/>

### 07
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-07"}
  codeBlockMaximumHeight="250px"
  minimumCodeHeight="250px"
/>

### 08
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-08"}
  codeBlockMaximumHeight="250px"
  minimumCodeHeight="250px"
/>

### 09
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-09"}
  codeBlockMaximumHeight="250px"
  minimumCodeHeight="250px"
/>

### 10
<CodePreview
  sourcePath="src/components/switchs"
  componentName={"switch-10"}
  codeBlockMaximumHeight="250px"
  minimumCodeHeight="250px"
/>

### more...
<ComponentPage 
  sourcePath="src/components/switchs"
  files={[
    "switch-11",
    "switch-12",
    "switch-13",
    "switch-14",
    "switch-15",
    "switch-16",
    "switch-17"
  ]}
/>

## Options
| Component | Description | Example Usage |
| --------- | ----------- | ------------- |
| `Switch`  | A toggle component that allows users to switch between two states, such as on and off. | `<Switch defaultChecked />` |
